...
describe('Check-in', () => {
    ...
    it('Validar bread check-in', () => {
        ...
        .should('be.visible')
        .click()
        cy.url().should('include', 'https://amei-homolog.amorsaude.com.br/schedule/checkin')
    });
    ...
    it.only('Validar Fluxo Checkin no PIX (Não tef)', () => {
        const baseUrl = Cypress.env('currentBaseUrl');
        cy.visit(baseUrl);
        cy.get('#schedule', { timeout: 15000 }).click()
        cy.get('span').contains('Check-in').click()
    ...
    it.only('Validar Fluxo Checkin no Dinheiro (Não tef)', () => {
        const baseUrl = Cypress.env('currentBaseUrl');
        cy.visit(baseUrl);
        cy.get('#schedule', { timeout: 15000 }).click()
        cy.get('span').contains('Check-in').click()
...
describe('Proposta', () => {
    ...
    it('Validar Fluxo de Proposta para 1 exame com pagamento no Dinheiro com busca por Nome (Não tef)', () => {
        ...
        cy.xpath("//input[contains(@aria-required,'false')]").click({ force: true }).type('Ivan Aparecido Barros Dos Santos');
    ...
    it('Validar Fluxo de Proposta para 1 exame com pagamento no Credito com busca por Nome (Não tef)', () => {
        ...
        cy.xpath("(//button[contains(.,'edit')])[3]").click()
    ...
    it('Validar Fluxo de Parcelas na tela Cartões', () => {
        ...
        cy.get('tbody > tr > .cdk-column-valuePayer', timeout)
            ...
            .then(() => {
                ...
                cy.get('[style="background-color: #fff; width: 100%;"] > div.ng-tns-c1537010922-8 > :nth-child(1) > .ng-tns-c1537010922-8')
                    .invoke('text')
                    .then(valorBrutoText => {
